...
import { faqs } from "@/lib/chatbot/knowledge";
import { PERSONA } from "@/lib/chatbot/persona";

export const runtime = "nodejs";

// Uses Groq's free-tier API — OpenAI-compatible format, no extra npm
// dependency needed beyond a plain fetch call.
...
// reuse one Groq call instead of paying for it twice within the TTL window.
...
// Circuit breaker: if Groq starts failing repeatedly (rate limit, outage, a
...
function buildSystemPrompt(): string {
  ...
  return `You are the assistant for SolarLeadAds.com, a Meta (Facebook/Instagram) ads lead-generation agency built only for the solar industry. Every visitor is a solar business, not a homeowner.

Answer questions about pricing, leads, regions covered, Meta ad access, and the onboarding process using ONLY the information below. If asked something outside this scope, say you're not sure and offer to connect them with the team via the contact form. Keep answers short: 2 to 4 sentences, unless listing pricing tiers.
...
Regions served: ${regions.map((r) => r.label).join(", ")} (plus other countries on a case-by-case "Global" basis).

Pricing by track and region:
${pricingText}
